...
namespace CrossCraft
{
    ...
    auto TexturePackManager::load_texture(std::string filename, u32 magFilter,
        u32 minFilter, bool repeat, bool flip) -> u32 {
        ...
        for (const auto &entry : std::filesystem::directory_iterator(p))
        {
            ...
            else if (entry.is_regular_file())
            {
                ...
                if (filename.find(".zip") != std::string::npos)
                {
                    ...
                    path_names.push_back(entry.path().filename().string().substr(0, filename.find(".zip")));
                }
            }
        }
    ...
    auto TexturePackManager::extract_zip(std::string path) -> int
    {
        ...
        for (int i = 0; i < global_info.number_entry; i++) {
            ...
            std::string prefix_name = path.substr(0, path.find(".zip")) + "/" + std::string(filename);
